...
Outputs: 
  Mode 1:
    Posterior_results (.csv + .json)
  
  Mode 2:
    Posterior_results (.csv + .json)
    objective (.csv + .json)
    Probability of 0.5 % prevalence rate across sites (.csv + .json)
    Probability of 1 % prevalence rate across sites (.csv + .json)
    Probability of disease free across sites (.csv + .json)
    sample_efforts (.csv + .json)
    sample_size  (.csv + .json)
...
def compute_posteriors(inc_df: pd.DataFrame,
    ...
    for _, row in inc_df.iterrows():
        sid = row['_id']
        theta0 = float(row['introduction rate'])
...
def run_posterior_only(mode_df, inc_df, hist_cols):
    ...
    json_path = RESULTS_DIR / "posterior_results.json"
...
def run_growth_optimization(mode_df, inc_df, hist_cols):
    ...
    json_post = RESULTS_DIR / "posterior_results.json"

    df_post.to_csv(csv_post, index=False)
    add_item_to_json_file_list(
        ATTACHMENTS_JSON,
        {"filename": csv_post.name, "content_type": "text/csv", "role": "posterior"}
    )
    df_post.to_json(json_post, orient='records', indent=2)
    add_item_to_json_file_list(
        ATTACHMENTS_JSON,
        {"filename": json_post.name, "content_type": "application/json", "role": "posterior"}
    )
    logger.info("The posterior update completed successfully")
    ...
    th_s   = inc_df.set_index('_id')['introduction rate']
    ...
    try:
        time, u_vals, P0_vals, P1_vals, P2_vals, obj = extract_results_pyomo(model,verbose=True)
    ...
    # Save objective
    df_obj = pd.DataFrame({"objective": [obj]})
    csv_obj = RESULTS_DIR / "objective.csv"
    json_obj = RESULTS_DIR / "objective.json"
    samples_by_site = {
    ...
    add_item_to_json_file_list(
        ATTACHMENTS_JSON,
        {"filename": csv_obj.name, "content_type": "text/csv", "role": "objective"}
    )
    df_obj.to_json(json_obj, orient='records', indent=2)
    add_item_to_json_file_list(
        ATTACHMENTS_JSON,
        {"filename": json_obj.name, "content_type": "application/json", "role": "objective"}
    )
    ...
    def save_wide(data, name, period, transform=None):
        ...
        jsonw = RESULTS_DIR / f"{name}.json"

        dfw.to_csv(csvw, index=False)
        add_item_to_json_file_list(
            ATTACHMENTS_JSON,
            {"filename": csvw.name, "content_type": "text/csv", "role": name}
        )
        dfw.to_json(jsonw, orient='records', indent=2)
        add_item_to_json_file_list(
            ATTACHMENTS_JSON,
            {"filename": jsonw.name, "content_type": "application/json", "role": name}
        )
    ...
    # probabilities
    save_wide(P0_vals, "Probability of disease free across sites", time)
    save_wide(P1_vals, "Probability of 0.5 % prevalence rate across sites", time)
    save_wide(P2_vals, "Probability of 1 % prevalence rate across sites", time)
...
def run_cost_analysis(mode_df, inc_df, hist_cols):
    ...
    for d in discounts:
        ...
        try:
            ...
            sys.exit(32)
        results3.append({'Percentage': d, 'amount': bud, 'objective': obj})
    ...
    add_item_to_json_file_list(
        ATTACHMENTS_JSON,
        {"filename": csv_ca.name, "content_type": "text/csv", "role": "cost_analysis"}
    )
    # Save JSON
    json_ca = RESULTS_DIR / "cost_analysis.json"
    df3.to_json(json_ca, orient="records", indent=2)
    add_item_to_json_file_list(
        ATTACHMENTS_JSON,
        {"filename": json_ca.name, "content_type": "application/json", "role": "cost_analysis"}
    )
...
def main():
    ...
    inc_df = sample_df[sample_df['Consider'].str.lower() == "include"]
    if len(inc_df) < 2:
        logger.error(
